...
public:
	...
	int altura(BBTree<T>* Altura);
	void escriureFitxerRec(ofstream& fitxer, BBTree<T>* bb, int i, int imax);
...
template <class T>
int BBTree<T>::altura(BBTree<T>* Altura)
{
	int esquerra = 0;
	int dreta = 0;

	if (Altura->getLeft() == NULL && Altura->getRight() == NULL) { return 0; }
	else
	{
		if (Altura->getLeft() != NULL)
		{
			esquerra = altura(Altura->getLeft());
		}

		if (Altura->getRight() != NULL)
		{
			dreta = altura(Altura->getRight());
		}
		if (esquerra > dreta)
		{
			esquerra++;
...
template <class T>
void BBTree<T>::escriureFitxerRec(ofstream& fitxer, BBTree<T>* m_clauer, int i, int imax)
{
	BBTree<T>* pAct = m_clauer;

	if ((pAct != NULL))
	{
		fitxer << "1" << " ";
		fitxer << pAct->getData();
		escriureFitxerRec(fitxer, pAct->getLeft(), i + 1, imax);
		escriureFitxerRec(fitxer, pAct->getRight(), i + 1, imax);
	}
	else if (i <= imax)
	{
		fitxer << "0" << endl;
